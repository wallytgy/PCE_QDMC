...
%% Simulation 1: no higher order weights
...
y_min = -0.02 *ones(15,1);   % Minimum output
y_max = 0.02*ones(15,1);    % Maximum output
...
c_C_desired = [ 0.008* ones(1,t_simulation/dt) ; zeros(14,t_simulation/dt)];
...
print(gcf, '-depsc', 'varying_sp_nd_case_1_output_novarweight.eps');
c_C_store_all_nd = c_C_store_all
xregion(50,150,'DisplayName',"First Output Fault","FaceColor",'w')
xregion(250,350,'DisplayName',"Second Output Fault","FaceColor",'w')
%%
figure(6)
for iteration = 1:N_iter
newHue = [0.8 0.1 0.1];    % light teal (R‑G‑B between 0‑1)
plot(tsp_sim(1:end-1), squeeze(u_store_all(iteration,1,:)), 'Color', newHue, 'LineWidth',0.5);
xlabel('Time (s)')
ylabel('u (L/s)')
hold on
end
xregion(50,150,'DisplayName',"First Output Fault","FaceColor",'w')
xregion(250,350,'DisplayName',"Second Output Fault","FaceColor",'w')
print(gcf, '-depsc', 'varying_sp_nd_case_1_control_novarweight.eps');
...
y_store = zeros(n_out,length(tsp_sim));     % Outputs are measured at t
u_store = zeros(n_in,length(tsp_sim)-1);
c_C_store = zeros(length(tsp_sim),1); % Inputs are applied over delta t
% Define states and outputs for t=0
x_dev = zeros(n_x,1);   % Start the process from steady state (deviation variable)
y = zeros(n_out,1); % Start the process from steady state (deviation variable)
% Write value for t=0 to storage
...
figure(7)
for iteration = 1:N_iter
plot(tsp_sim, c_C_store(:,1), 'g' , tsp_sim(2:end),  ysp(1,:) + x_ss(3), 'b')
xlabel('Time (s)')
ylabel('c_C (mol/L)')
hold on
end
...
print(gcf, '-depsc', 'varying_sp_id_case_1_output_novarweight.eps');
...
xregion(50,150,'DisplayName',"First Input Fault","FaceColor",'r')
xregion(250,350,'DisplayName',"Second Input Fault","FaceColor",'r')
print(gcf, '-depsc', 'varying_sp_id_case_1_control_novarweight.eps');
...
%% plot output response
figure(9)
plot(tsp_sim, c_C_store(1,:), 'magenta' , tsp_sim(2:end),  ysp(1,:) + x_ss(3), 'b')
xlabel('Time (s)')
ylabel('c_C (mol/L)')
hold on
end 
...
print(gcf, '-depsc', 'varying_sp_od_case_1_output_novarweight.eps');

%%
figure(10)
for iteration = 1:N_iter
newHue = [0.1 0.8 0.1];    % light teal (R‑G‑B between 0‑1)
plot(tsp_sim(1:end-1), squeeze(u_store_all(iteration,1,:)), 'Color', 'magenta', 'LineWidth',0.5);
ylim([-3,3])
xlabel('Time (s)')
ylabel('u (L/s)')
hold on
...
xregion(50,150,'DisplayName',"First Input Fault","FaceColor",'r')
xregion(250,350,'DisplayName',"Second Input Fault","FaceColor",'r')
print(gcf, '-depsc', 'varying_sp_od_case_1_control_novarweight.eps');
