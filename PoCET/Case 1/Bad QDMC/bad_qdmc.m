...
basis_samples = PoCETsample(sys,'basis',mc_samples);
histogram_data_galerkin = results.c_C.pcvals(:,end)' *  basis_samples

mcresults_for_histogram = PoCETsimMonteCarlo(sys,'my_MCODE',[],samples,simoptions,'method','final');
histogram_data_mc = mcresults_for_histogram.c_C.mcvals(:)

%% plot data
figure(3)
histogram(histogram_data_galerkin)
hold on
histogram(histogram_data_mc)
xlabel("$c_C$ (L/s)", "Interpreter","Latex")
ylabel("Frequency")
legend("PCE Expansion", "Monte-Carlo")
var_PCE = var(histogram_data_galerkin, 1);   % population variance
var_MC  = var(histogram_data_mc,        1);
absDiff = abs(var_MC - var_PCE);
relDiff = absDiff / max(var_MC, eps);        % avoid div‑by‑zero
...
PAR.k_1 = (0.278900000000000 + 0.892700000000000)/2
PAR.k_2 = (0.189400000000000 + 0.933100000000000)/2
x_ss_nominal = fsolve(@(x) my_MCODE(0,x,PAR,inputs(1).u_t,inputs(1).u_v) , [X_ss(1),X_ss(16),X_ss(31),X_ss(46)]);
...
Su = reshape(S_yu, [1,15,1,20]);
for i = 1:20
    Su(1,:,1,i) = Su(:,1,i) - X_ss(31:45);
end
...
n_in = 1;%   n_in        Number of input variables, scalar
n_out = 1;%   n_out       Number of output variables, scalar
L = 15;%   L           Number of terms in the ploynomial chaos expansion (PCE),scalar
IP = ones(15,1);%   IP          Inner product of basis function of the PCE with themselves,
%               IP(j) = <phi(j-1),phi(j-1)> since the index starts from 1 
%               while the nomenclature starts from 0, vector [L by 1]
n_dis = 0% * n_dis *     Number of measured disturbances, scalar
%   Su          Step response coefficients input, matrix,
%               [n_out by L by n_in by n] -> 4D matrix
Sd = [];% * Sd *        Step response coefficients disturbances, matrix,
%               [n_out by L by n_dis by n] -> 4D matrix
p = 20; %   p           Prediction horizon, scalar
c = 10; %   c           Control horizon, scalar
La = 1; % * La *        Vector of control weight parameters, 
%               i.e., J = ||r-y|| + La*eye*||du|| where du is the input
%               change, [n_in by 1]
Q = 1e3;% * Q *         Vector of output weight parameters, vector [n_out by 1]
% * ctg *       Vector of cost to go parameters (additional weight for the
%               last predicted output error, vector [n_out by 1]
Qvar = 1; % * Qvar *      Vector of variance weight parameters, vector [n_out by 1]
u_min = -2.5;     % Minimum input
u_max = 2.5;    % Maximum input
D_u_min = -0.1; % Minimum input movement
D_u_max = 0.1;  % Maximum input movement
y_min = -0.02;   % Minimum output
y_max = 0.02;    % Maximum output
ctg = [];       % Cost to go
...
N_iter = 100;
c_C_store_all = zeros(N_iter , length(tsp_sim));
u_store_all = zeros(N_iter,n_in, length(tsp_sim)-1)
%%% find steady state of current system
for iteration = 1:N_iter


%%% steady state for the parameter realization
...
% setpoint to run closed loop simulation
c_C_desired = [ 0.008* ones(1,t_simulation/dt)];
c_C_desired(:,201:400) = 0.5 * c_C_desired(:,201:400);
c_C_desired(:,601:800) = 0.5 * c_C_desired(:,601:800);
ysp = c_C_desired;
ysp(1,:) = ysp(1,:) - x_ss(3);
udist = [];
...
Controller = SQDMC_controller(n_in,n_out,L,IP,n_dis,Su,Sd,p,c,La,Q,ctg,Qvar,...
...
c_C_store = zeros(1,length(tsp_sim)); % Inputs are applied over delta t
...
for ii = 1:length(tsp_sim)-1
    ...
    c_C_dev = x_dev(3);
    y = c_C_dev;   % Process output
    
    % Store results
    x_store(:,ii+1) = x_dev;
    y_store(:,ii+1) = y;
    u_store(:,ii) = u;
    c_C_store(ii+1) = x_ss(3) + y(1);
    u_store_all(iteration,:,ii) = u;
    
end
c_C_store_all(iteration,:) = c_C_store;
end 
%% plot output response
figure(5)
for i = 1:N_iter
plot(tsp_sim, c_C_store_all(i,:), 'r' , tsp_sim(2:end),  ysp(1,:) + x_ss(3), 'b')
xlabel('Time (s)')
ylabel('c_C (mol/L)')
ylim([0,0.02])
hold on
end
c_C_store_all_nd = c_C_store_all
print(gcf, '-depsc', 'varying_sp_nd_case_1_output_bad.eps');


%%
u_store_all_nd = u_store_all
figure(6)
for iteration = 1:N_iter
newHue = [0.8 0.1 0.1];    % light teal (R‑G‑B between 0‑1)
plot(tsp_sim(1:end-1), squeeze(u_store_all_nd(iteration,1,:)), 'Color', newHue, 'LineWidth',0.5);
xlabel('Time (s)')
ylabel('u (L/s)')
hold on
end
xregion(50,150,'DisplayName',"First Input Fault","FaceColor",'w')
xregion(250,350,'DisplayName',"Second Input Fault","FaceColor",'w')
print(gcf, '-depsc', 'varying_sp_nd_case_1_control_bad.eps');
...
%% Perform closed loop control for varying paramters========================================== %
% Time-varying disturbance turned on from t\in[50,150] and t\in[250,350]
u_dist = 0.5 * [zeros(1,100) ones(1,200) zeros(1,200) -1*ones(1,200) zeros(1,100)];
N_iter = 100;
c_C_store_all = zeros(N_iter , length(tsp_sim));
u_store_all = zeros(N_iter,n_in, length(tsp_sim)-1)
%%% find steady state of current system
...
figure(7)
for i = 1:N_iter
plot(tsp_sim, c_C_store_all(i,:), 'g' , tsp_sim(2:end),  ysp(1,:) + x_ss(3), 'b')
xlabel('Time (s)')
ylabel('c_C (mol/L)')
...
xregion(50,150,'DisplayName',"First Output Fault","FaceColor",'r')
xregion(250,350,'DisplayName',"Second Output Fault","FaceColor",'r')
print(gcf, '-depsc', 'varying_sp_id_case_1_output_bad.eps');
c_C_store_all_id = c_C_store_all;

%%
u_store_all_id = u_store_all
figure(8)
for iteration = 1:N_iter
newHue = [0.1 0.8 0.1];    % light teal (R‑G‑B between 0‑1)
plot(tsp_sim(1:end-1), squeeze(u_store_all_id(iteration,1,:)), 'Color', newHue, 'LineWidth',0.5);
xlabel('Time (s)')
ylabel('u (L/s)')
hold on
end
...
print(gcf, '-depsc', 'varying_sp_id_case_1_control_bad.eps');
...
y_dist = 0.003 * [zeros(1,100) ones(1,200) zeros(1,200) -1*ones(1,200) zeros(1,100)];
N_iter = 100
c_C_store_all = zeros(N_iter , length(tsp_sim));
u_store_all = zeros(N_iter,n_in, length(tsp_sim)-1)
%%% find steady state of current system
...
figure(9)
for i = 1:N_iter
plot(tsp_sim, c_C_store_all(i,:), 'magenta' , tsp_sim(2:end),  ysp(1,:) + x_ss(3), 'b')
xlabel('Time (s)')
ylabel('c_C (mol/L)')
hold on
end
xregion(50,150,'DisplayName',"First Output Fault","FaceColor",'r')
xregion(250,350,'DisplayName',"Second Output Fault","FaceColor",'r')
print(gcf, '-depsc', 'varying_sp_od_case_1_output_bad.eps');




figure(10)
u_store_all_od = u_store_all
for iteration = 1:N_iter
plot(tsp_sim(1:end-1), squeeze(u_store_all_od(iteration,1,:)), 'Color', 'magenta', 'LineWidth',0.5);
xlabel('Time (s)')
ylabel('u (L/s)')
hold on
end
xregion(50,150,'DisplayName',"First Input Fault","FaceColor",'r')
xregion(250,350,'DisplayName',"Second Input Fault","FaceColor",'r')
print(gcf, '-depsc', 'varying_sp_od_case_1_control_bad.eps');

c_C_store_all_od = c_C_store_all


%%
c_C_store_all_ = permute(c_C_store_all_nd,[2,1]);
IAE_nd = computeAvgIAE_single(c_C_store_all_(2:end,:), c_C_desired(1,:)')
width_nd = computeEnvelopeWidth_single(c_C_store_all_(2:end,:))

c_C_store_all_ = permute(c_C_store_all_id,[2,1]);
IAE_id = computeAvgIAE_single(c_C_store_all_(2:end,:), c_C_desired(1,:)')
width_id = computeEnvelopeWidth_single(c_C_store_all_(2:end,:))

c_C_store_all_ = permute(c_C_store_all_od,[2,1]);
IAE_od = computeAvgIAE_single(c_C_store_all_(2:end,:), c_C_desired(1,:)')
width_od = computeEnvelopeWidth_single(c_C_store_all_(2:end,:))