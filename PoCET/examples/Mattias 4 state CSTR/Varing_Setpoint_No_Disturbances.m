...
q_2 = 1
V = 5
k_3 = 0.3264 % rmbr to change the eqns themselves
k_4 = 0.01591
c_Ain = 1
c_Bin = 7.35
...
states(2).dist = 'none';
states(2).data = 3.5;
states(2).rhs = '1/5*7.35 - (2.75+u+1)/5*c_B - c_A*c_B*(k_1+k_2) - c_B*c_C*0.3264 - c_B*c_D*0.01591';

states(3).name = 'c_C';
states(3).dist = 'none';
states(3).data = 0;
states(3).rhs = '-(2.75+u+1)/5*c_C + c_A*c_B*k_1 - c_B*c_C*0.3264'
...
y_min = -0.5 *ones(15,1);   % Minimum output
y_max = 0.5*ones(15,1);    % Maximum output
...
c_C_desired = [ 1.1 * X_ss(31)* ones(1,t_simulation/dt) ; zeros(14,t_simulation/dt)];
...
figure(5)
plot(tsp_sim, c_C_store(:,1), 'r' , tsp_sim(2:end),  ysp(1,:) + x_ss(3), 'b')
xlabel('Time (s)')
ylabel('c_C (L/s)')
hold on
end 
