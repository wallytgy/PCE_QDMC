...
states(1).dist = 'none';
states(1).data = 0;
states(1).rhs = '(2.75+u)/5*0.01 -(2.75+u+1)/5*c_A -c_A*c_B*(k_1+k_2)';

states(2).name = 'c_B';
states(2).dist = 'none';
states(2).data = 3.5;
states(2).rhs = '1/5*7.35 - (2.75+u+1)/5*c_B - c_A*c_B*(k_1+k_2) - c_B*c_C*0.003264  - c_B*c_D*0.01591';

states(3).name = 'c_C';
states(3).dist = 'none';
states(3).data = 0;
states(3).rhs = '-(2.75+u+1)/5*c_C + c_A*c_B*k_1 - c_B*c_C*0.003264 '
...
histogram_data_galerkin = results.c_D.pcvals(:,end)' *  basis_samples

mcresults_for_histogram = PoCETsimMonteCarlo(sys,'my_MCODE',[],samples,simoptions,'method','final');
histogram_data_mc = mcresults_for_histogram.c_D.mcvals(:)
...
xlabel("$c_D$ (L/s)", "Interpreter","Latex")
ylabel("Frequency")
...
[n_r , ~] = size(X_response);
% y = c_D, u = F
c_D_PCE = X_response((n_r/4) * 3 + 1: (n_r/4) * 4, :);
S_yu = c_D_PCE(:,6:5:101);
Su = reshape(S_yu, [15,1,20]);
for i = 1:20
    Su(:,1,i) = Su(:,1,i) - X_ss(46:60);
end
...
Q = (1./X_ss(46:60).^2);  % Weight for output error
for i = 2:15
    Q(i) = Q(i)* 0.00;
end
...
u_min = -10;     % Minimum input
u_max = 10;    % Maximum input
D_u_min = -0.5; % Minimum input movement
D_u_max = 0.5;  % Maximum input movement
...
c_D_desired = [ 0.001 * ones(1,t_simulation/dt) ; zeros(14,t_simulation/dt)];
c_D_desired(:,201:400) = 0.5 * c_D_desired(:,201:400);
c_D_desired(:,601:800) = 0.5 * c_D_desired(:,601:800);
ysp = c_D_desired;
ysp(1,:) = ysp(1,:) - x_ss(4);
udist = [];
...
c_D_store = zeros(length(tsp_sim)); % Inputs are applied over delta t
...
for ii = 1:length(tsp_sim)-1
    ...
    c_D_dev = x_dev(4);
    y = [c_D_dev ; zeros(14,1)];  % Process output
    
    % Store results
    x_store(:,ii+1) = x_dev;
    y_store(:,ii+1) = y;
    u_store(:,ii) = u;
    c_D_store(ii+1) = x_ss(4) + y(1);
end

%% plot output response
figure(5)
plot(tsp_sim, c_D_store(:,1), 'r' , tsp_sim(2:end),  ysp(1,:) + x_ss(4), 'b')
xlabel('Time (s)')
ylabel('c_C (L/s)')
hold on
end 
