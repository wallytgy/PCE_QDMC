...
% %%
% % ---------- INPUTS ----------
% % T        : n_MC × n_t × n_v        (Monte‑Carlo snapshots)
% % x_PCE    : n_t × n_v × n_PCE       (all PCE coeffs; coeff 1 = mean)
% % idx      : n_v × 1                 (cluster labels 1..k)
% 
% [n_MC, n_t, n_v] = size(x_responses_MC);
% [~,      ~, n_PCE] = size(x_PCE);
% k = max(idx);                       % #clusters
% colors = lines(k);
% 
% 
% % Containers
% mean_MC = zeros(k,n_t);  var_MC  = zeros(k,n_t);
% mean_PCE = zeros(k,n_t); var_PCE = zeros(k,n_t);
% 
% for c = 1:k
%     nodes = find(idx==c);                 % nodes in cluster c
%     % ---------- Monte‑Carlo ----------
%     Tc = x_responses_MC(:,:,nodes);                    % n_MC × n_t × |C|
%     mc_cluster_mean = squeeze(mean(Tc,3));% n_MC × n_t
%     mean_MC(c,:) = mean(mc_cluster_mean, 1);
%     var_MC(c,:)  = var (mc_cluster_mean, 0, 1);  % across MC reps
% 
%     % ---------- PCE ----------
%     % coeff‑wise cluster averages
%     mu_c  = mean(x_PCE(:,nodes,1), 2);            % n_t ×1   (mean part)
%     a_ci  = squeeze(mean(x_PCE(:,nodes,2:end),2));% n_t ×(n_PCE-1)
%     mean_PCE(c,:) = mu_c';                        % row vector
%     var_PCE(c,:)  = sum(a_ci.^2, 2)';             % row vector (variance)
% end
% 
% % ========= 1) MC mean =========
% figure; hold on;
% for c=1:k
%     plot(1:n_t, mean_MC(c,:), 'Color',colors(c,:), 'LineWidth',2);
%     plot(1:n_t, mean_PCE(c,:), '--', 'Color',colors(c,:), 'LineWidth',2);
% end
% xlabel('time step'); ylabel('cluster mean'); title('MC – mean');
% legend("C"+(1:k),'Location','best'); grid on;
% 
% % ========= 2) MC variance =========
% figure; hold on;
% for c=1:k
%     plot(1:n_t, var_MC(c,:), 'Color',colors(c,:), 'LineWidth',2);
%     plot(1:n_t, var_PCE(c,:), '--', 'Color',colors(c,:), 'LineWidth',2);
% end
% xlabel('time step'); ylabel('variance'); title('MC – variance');
% legend("C"+(1:k),'Location','best'); grid on;
% 
% % ========= 3) PCE mean =========
% figure; hold on;
...
%     plot(1:n_t, mean_PCE(c,:), '--', 'Color',colors(c,:), 'LineWidth',2);
% end
% xlabel('time step'); ylabel('cluster mean'); title('PCE – mean');
% legend("C"+(1:k),'Location','best'); grid on;
% 
% % ========= 4) PCE variance =========
% figure; hold on;
...
%     plot(1:n_t, var_PCE(c,:), '--', 'Color',colors(c,:), 'LineWidth',2);
% end
% xlabel('time step'); ylabel('variance'); title('PCE – variance');
% legend("C"+(1:k),'Location','best'); grid on;
n_t = 1001
%%

[varDiff, relDiff] = plot_histograms_finalStep(PCE_polynomials, x_responses_MC, x_PCE, idx,mc_samples)
...
%%
plot_trajectories(y_store_all, ysp)
%%
% 2) Shaded min–max envelopes plus mean trajectories
plotClusterTrajectories(u_store_all, flat_reps, representatives, tsp_sim,...
                        'PlotStyle','envelope','Opacity',0.25,'ShowMean',true);



%%
% 2) Shaded min–max envelopes plus mean trajectories
plotClusterTrajectories(u_store_all, flat_reps, representatives, tsp_sim, "varying_sp_id_case_2_control",...
...
function [varDiff, relDiff] = plot_histograms_finalStep( ...
        thetaBasis, x_responses_MC, x_PCE, idx, nSamples)
% -------------------------------------------------------------------------
% Overlays MC vs PCE histograms for every cluster (final time step)
% and returns variance differences.
%   varDiff(c) = |var_MC – var_PCE|   for cluster c
%   relDiff(c) = varDiff(c) / var_MC  (fractional)
% -------------------------------------------------------------------------
if nargin < 5, nSamples = 1e4; end        % default #θ‑samples
% ------------ dimensions & basis function handles ------------------------
[n_MC,  ~]      = size(x_responses_MC);
[n_t,  ~, nPCE] = size(x_PCE);
k              = max(idx);                % # clusters
theta1 = sym('theta1'); theta2 = sym('theta2');
phiFun = arrayfun(@(p) matlabFunction(p,'Vars',[theta1 theta2]), ...
                  thetaBasis(:),'UniformOutput',false);
% ------------ draw θ‑samples once, build Φ --------------------------------
th1 = -1 + 2*rand(nSamples,1);            % U[-1,1]
th2 = -1 + 2*rand(nSamples,1);
Phi = zeros(nSamples,nPCE);
for i = 1:nPCE
    Phi(:,i) = phiFun{i}(th1,th2);
end
t_final = n_t;                            % last time slice
% ------------ allocate outputs -------------------------------------------
varDiff = zeros(k,1);
relDiff = zeros(k,1);
% ------------ create tiled figure ----------------------------------------
...
title(tl,'MC vs PCE histograms');
for c = 1:k
    nexttile;  hold on;
    nodes   = (idx == c);
    % Monte‑Carlo values (cluster‑mean per run)
    mc_vals  = mean(x_responses_MC(:,nodes),2);
    % PCE values (sampled)
    coeff_c  = squeeze(mean(x_PCE(t_final,nodes,:),2));
    pce_vals = Phi * coeff_c;
    % ---------- variance gap ---------------------------------------------
    var_MC  = var(mc_vals,  1);           % population variance
    var_PCE = var(pce_vals, 1);
    varDiff(c) = abs(var_MC - var_PCE);
    relDiff(c) = varDiff(c) / max(var_MC, eps);  % avoid /0
    % ---------- histograms -----------------------------------------------
    nbins = 60;
    edges = linspace(min([mc_vals; pce_vals]), ...
                     max([mc_vals; pce_vals]), nbins+1);
    histogram(pce_vals,'BinEdges',edges,'FaceAlpha',0.5,...
                  'FaceColor',[0 0.45 0.74],'EdgeColor','none');
    histogram(mc_vals ,'BinEdges',edges,'FaceAlpha',0.5,...
                  'FaceColor',[0.85 0.33 0.10],'EdgeColor','none');
    title(sprintf('Cluster %d',c));
    xlabel('value [K]');  ylabel('freq');
end
print(gcf,'MC_vs_PCE_case_2A','-depsc2','-vector');
end

function plot_trajectories(y_store_all, ysp)
    [xxx xxxx iterations] = size(y_store_all);
    t  = 0:400;                      % time vector (adjust if needed)
    col = [1 0 0];             % base colour for the trajectories
    ...
    for r = 1:6
        ...
        for it = 1:iterations
            plot(t, squeeze(y_store_all(r, :, it)), 'g'); 
        end
        plot(t(1:end-1), squeeze(ysp(r, :)), 'b');
    ...
    print(gcf, 'varying_sp_id_case_2', '-depsc2', '-vector'); 
end
