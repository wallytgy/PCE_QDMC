...
plot_histograms_finalStep(PCE_polynomials, x_responses_MC, x_PCE, idx,mc_samples)
...
plotClusterTrajectories(u_store_all, flat_reps, representatives, tsp_sim,"varying_sp_od_case_2_control",...
...
function plot_histograms_finalStep(thetaBasis, x_responses_MC, x_PCE, idx, nSamples)
    % ------------------------------------------------------------------------
    % Overlays MC vs PCE histograms for every cluster (final time step)
    % in a single 3 × 2 figure.
    % ------------------------------------------------------------------------
    if nargin < 5, nSamples = 1e4; end            % default #θ-samples
    
    % ------------ dimensions & basis function handles -----------------------
    [n_MC, n_v]        = size(x_responses_MC);
    [n_t,  ~, n_PCE]   = size(x_PCE);
    k                  = max(idx);                % should be 6
    ...
    for c = 1:k
        ...
        xlabel('value');  ylabel('freq');
            
    end
   print(gcf, 'MC_vs_PCE_case_2', '-depsc2', '-vector'); 
end
...
function plot_trajectories(y_store_all, ysp)
    ...
    for r = 1:6
        ...
        for it = 1:iterations
            plot(t, squeeze(y_store_all(r, :, it)), 'magenta');   
        end
    ...
    print(gcf, 'varying_sp_od_case_2a', '-depsc2', '-vector'); 
end
