...
tspan = breakpts(1):(breakpts(end)-breakpts(1))/(n_t-1):breakpts(end);
step_responses = zeros(length(flat_reps), n_t, k , N);
for i = 1:length(flat_reps)
    ...
    for c = 1:k
        ...
        step_responses(i,:,c,:) = output_c;
        
    end
end  
y_responses = permute(step_responses, [3 4 1 2]);
...
%%
plot_trajectories(y_store_all, ysp, "varying_sp_nd_case_2_output_bad.eps","r",false)

y_store_all_nd = y_store_all
% 2) Shaded min–max envelopes plus mean trajectories
plotClusterTrajectories(u_store_all, flat_reps, representatives, tsp_sim, "varying_sp_nd_case_2_control_bad.eps",...
...
%%
plot_trajectories(y_store_all, ysp, "varying_sp_id_case_2_output_bad.eps" , "g", true)

y_store_all_id = y_store_all
% 2) Shaded min–max envelopes plus mean trajectories
plotClusterTrajectories(u_store_all, flat_reps, representatives, tsp_sim, "varying_sp_id_case_2_control_bad.eps",...
                        'PlotStyle','lines','Opacity',0.1,'ShowMean',true);
...
%%
plot_trajectories(y_store_all, ysp , "varying_sp_od_case_2_output_bad.eps" , "magenta", true)
y_store_all_od = y_store_all
%%
% 2) Shaded min–max envelopes plus mean trajectories
plotClusterTrajectories(u_store_all, flat_reps, representatives, tsp_sim,"varying_sp_od_case_2_control_bad.eps",...
                        'PlotStyle','lines','Opacity',0.1,'ShowMean',true);
